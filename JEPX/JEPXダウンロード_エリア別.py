...
#    print(df)
    ...
    try:
        path_JEPX = os.path.dirname(os.path.abspath(__file__)) #このファイルのあるディレクトリのパスを取得
        os.chdir(path_JEPX+'\\'+area+'確定価格')
        df.to_csv(filename,encoding="shift_jis")
        #shutil.move(filename, folder_jepx)#各エリアのフォルダに移動させる
        #os.remove(filename)#最初にダウンロードしたファイルを削除
    except:#もしすでにあったら、
        print('すでにファイルが存在します')
...
if __name__ == '__main__':
    ...
    while True:
        try:
            downloadfilename ='spot_'+str(now.year)+'.csv'#2021年なら、spot_2021.csvというファイル名でダウンロードされるので、それに合わせる
    ...
    time.sleep(1)
    if os.path.exists('spot_'+str(now.year)+'.csv.crdownload')== True:
        os.remove('spot_'+str(now.year)+'.csv.crdownload')
