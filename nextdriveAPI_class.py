...
from nextdriveAPI_functions import DataRetrieval_30min
from nextdriveAPI_functions import store_SolarPW,store_SmartMeter,store_AirCon,store_StgBattery,store_thermometer
from nextdriveAPI_functions import Control_AirCon,Control_StgBattery,Control_ElecWaterHeater
import configparser
import logging

config = configparser.ConfigParser()
config.read('config.ini', encoding='utf-8')
BASE_URL_NextDrive = config['NextDrive']['BASE_URL_NextDrive']
...
class House_init:#Uuidを取得する前のクラス
    ...
    def devices(self):
        devices_url = BASE_URL_NextDrive + self.id +'/devices'
    ...
    def store_Uuid(self):
        ...
        try:
            ...
            print('クラスHouse_initの関数store_Uuid:',self.id,'はidな不正なものです')
        else:
            ...
            dict = dict['Uuid']
            # 存在チェック
            path_user = USER_INFORMATION_FOLDER + '\\' + self.name + '_' + self.id
            ...
            if not os.path.exists(path_user):
                ...
                os.chdir(path_user)
                filename = '取得のままの'+'Uuid_'+self.name +'_'+self.id+'.csv'
                ...
                filename_json = 'Uuid_'+self.name +'_'+self.id+'.json'
...
class House:
    def __init__(self,name,id):
        ...
        try:
            ...
            print('クラスHouseの_init_:',id,'は不正なidです')
        else:
            filename_json = 'Uuid_'+self.name+'_'+self.id+'.json'
            json_open = open(filename_json, 'r')
            json_load = json.load(json_open)
            self.Uuid = json_load#辞書型
    #メソッドを定義する。
    def devices(self):
        devices_url = BASE_URL_NextDrive + self.id +'/devices'
        response = requests.get(devices_url,headers=headers)
    ...
    def SmartMeter(self):
        if dict_SmartMeter(self.Uuid) == False:
            return False
        else:
            return 'SmartMeter',dict_SmartMeter(self.Uuid)

    def SolarPW(self):
        if dict_SolarPW(self.Uuid) == False:
            return False
        else:
            return 'SolarPW',dict_SolarPW(self.Uuid)

    def StgBattery(self):
        if dict_StgBattery(self.Uuid) == False:
            return False,False
        else:
            return 'StgBattery',dict_StgBattery(self.Uuid)

    def thermometer(self):
        if dict_thermometer(self.Uuid) == False:
            return False,False
        else:
            return 'thermometer',dict_thermometer(self.Uuid)

    def AirCon_living(self):
        if dict_AirCon_living(self.Uuid) == False:
            return False,False
        else:
            return 'AirCon_living',dict_AirCon_living(self.Uuid)

    def AirCon_bedroom(self):
        if dict_AirCon_bedroom(self.Uuid) == False:
            return False,False
        else:
            return 'AirCon_bedroom',dict_AirCon_bedroom(self.Uuid)

    def ElecWaterHeater(self):
        if dict_ElecWaterHeater(self.Uuid) == False:
            return False,False
        else:
            return 'ElecWaterHeater',dict_ElecWaterHeater(self.Uuid)            

    def DataRetrieval_SmartMeter_30min(self):
        if dict_SmartMeter(self.Uuid) == False:
            return False,False
        else:
            Uuid=dict_SmartMeter(self.Uuid)
            scope = ["instanceElectricity"]
            df,endtime = DataRetrieval_30min(self.id,Uuid,scope)
        return df,endtime
    ...
    def DataRetrieval_StgBattery_30min(self):
        if dict_StgBattery(self.Uuid) == False:
            ...
            Uuid=dict_StgBattery(self.Uuid)
    ...
    def Get_latest_StgBattery_info(self):
        if type(dict_StgBattery(self.Uuid)) is str:#StgBatteryがあれば、
            Uuid_StgBattery = dict_StgBattery(self.Uuid)        
            df_StgBattery,endtime = store_StgBattery_FOR_LATEST_INFO(self.id,Uuid_StgBattery)
            return df_StgBattery,endtime
        else:
            return 'no StgBattery'

    def Calc_latest_StgBattery_behavior(self):
        #ToDO:ShunsukeHata8010→分けて使うときは無いので、これとstore_StgBattery_FOR_COMPARISON_INFOを一緒にした方がすっきりする
        if type(dict_StgBattery(self.Uuid)) is str:#StgBatteryがあれば、
            Uuid_StgBattery = dict_StgBattery(self.Uuid)
            df_StgBattery,endtime = store_StgBattery_FOR_COMPARISON_INFO(self.id,Uuid_StgBattery)
    ...
    def DataRetrieval_thermometer_30min(self):
        if dict_thermometer(self.Uuid)==False:
            ...
            Uuid=dict_thermometer(self.Uuid)
    ...
    def DataRetrieval_AirCon_bedroom_30min(self):
        if dict_AirCon_bedroom(self.Uuid)==False:
            ...
            Uuid=dict_AirCon_bedroom(self.Uuid)
    ...
    def DataRetrieval_AirCon_living_30min(self):
        if dict_AirCon_living(self.Uuid) == False:
            ...
            Uuid = dict_AirCon_living(self.Uuid)
            scope =  ["operationStatus","operationMode","temperatureSetting"]
            df,endtime = DataRetrieval_30min(self.id,Uuid,scope)
    ...
    def DataRetrieval_ElecWaterHeater_30min(self):
        if dict_ElecWaterHeater(self.Uuid) == False:
            ...
            Uuid = dict_ElecWaterHeater(self.Uuid)
...
#SolarPW,スマメ,エアコン,蓄電池,温度計があれば、それらをこの順番で保存する。
# なければそれはスキップする。
    def store_30min(self):
        ...
        try:
            if  type(dict_SolarPW(self.Uuid)) is str:#SolarPWがあれば、
                Uuid_SolarPW = dict_SolarPW(self.Uuid)
                df_SolarPW,endtime = store_SolarPW(self.id,Uuid_SolarPW)

            if  type(dict_SmartMeter(self.Uuid)) is str:#SmartMeterがあれば、
                Uuid_SmartMeter =dict_SmartMeter(self.Uuid)
                df_SmartMeter,endtime = store_SmartMeter(self.id,Uuid_SmartMeter)

            if  type(dict_AirCon_living(self.Uuid)) is str:#AirCon_livingがあれば、
                Uuid_AirCon_living =dict_AirCon_living(self.Uuid)
                df_AirCon_living,endtime = store_AirCon(self.id,Uuid_AirCon_living)
            
            if  type(dict_AirCon_bedroom(self.Uuid)) is str:#AirCon_bedroomがあれば、
                Uuid_AirCon_bedroom =dict_AirCon_bedroom(self.Uuid)
                df_AirCon_bedroom,endtime = store_AirCon(self.id,Uuid_AirCon_bedroom)

            if  type(dict_StgBattery(self.Uuid)) is str:#StgBatteryがあれば、
                Uuid_StgBattery =dict_StgBattery(self.Uuid)
                df_StgBattery,endtime = store_StgBattery(self.id,Uuid_StgBattery)

            if  type(dict_thermometer(self.Uuid)) is str:#thermometerがあれば、
                Uuid_thermometer =dict_thermometer(self.Uuid)
                df_thermometer,endtime = store_thermometer(self.id,Uuid_thermometer)

            if  type(dict_ElecWaterHeater(self.Uuid)) is str:#ElecWaterHeaterがあれば、
                Uuid_ElecWaterHeater =dict_ElecWaterHeater(self.Uuid)
                df_ElecWaterHeater,endtime = store_ElecWaterHeater(self.id,Uuid_ElecWaterHeater)
