    ...
    if type(df_battery_latest_behabior) is str :
        ...
        if math.isnan(latest_SOC):#SOCがnan=取れていないときの指示
            ...
            if Warning == True:#警報が出ているとき
                ...
                print(name,"警報などが発出されています!→蓄電池への指示量(kWh):",acChargeAmountSetting)
                ...
                print(name,"警報は出てきません。→蓄電池への指示量(kWh):",instruction_to_battery)

                if solver_status == 1:
                    print(name,'最適化成功')
                elif solver_status == -1:
                    print(name,'最適化失敗（Infeasible）!!!!!!!!!!!!!!!!')
                else:
                    print(name,'最適化失敗(Undefined,Unbounded,Not Solved)')
                print(name,"計算結果:", pulp.LpStatus[solver_status])
            ...
            if now.hour == 23:
                ...
                time_next_0 = now.replace(hour=now.hour+1,minute=0, second=0, microsecond=0)
            #指示量なので、現在時刻と最も近いコマを選んだ（現在のコマとは異なる）上で、
            # その次のコマを選ぶ
            ...
            if min_time == abs(time_30-now):
                ...
                if time_30.hour == 23:
                    time = time_30.replace(hour=0) + timedelta(days=1)
                else:
            ...
            #ロガーの設定
            logger = getLogger('main_solver.calc_battery_direction')
            logger.info('{0}:{1}:{2}'.format(name,'蓄電池への指示量',value_1))
            ...
            if acChargeAmountSetting == 0 and acDischargeAmountSetting == 0:
                print(name,'指示充放電量: 0Wh',response)
            elif acChargeAmountSetting == 0:
                print(name,'指示充放電量: ',acDischargeAmountSetting,'Wh',response)
            elif acDischargeAmountSetting == 0:
                print(name,'指示充放電量: ',acChargeAmountSetting,'Wh',response)
