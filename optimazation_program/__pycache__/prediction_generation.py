...
def calc_generation(Ido,Keido,panel_kakudo,panel_houi,pv_yoryo,rekka,ΔT,kasekisai,effi_PCS,effi_trans,loss_haisen_teikaku,kage_x,kage_y,kage_h,kage_Φ):
    #os.chdir(os.path.dirname(os.path.abspath(__file__)))#これ入れとかないとずれることがある
    ...
    for time,suihei in zip(df['予報時刻'],df['日射量']):
        ...
        s = s.replace(s[10],' ')
        dt_now = dt.strptime(s, '%Y-%m-%d %H:%M:%S')
    ...
    os.chdir("C:\\Users\\f-apl-admin\\発電量予測\\学習元ファイル")
    ...
    for time in df['予報時刻']:
        s = time.replace('+09:00','')
        s = s.replace(s[10],' ')
        dt_now = dt.strptime(s, '%Y-%m-%d %H:%M:%S')#この時点ではdatetime型
...
if __name__ == '__main__':
    ...
    #変数関連
    Ido = 35.379438022304434
    Keido =139.55970598380688
    panel_kakudo=10 #パネル角度。度単位
    panel_houi=0 #方位。度単位
    pv_yoryo = 3.6  #PV容量[kW]
    kasekisai =1.0 #過積載率[小数表記！！]=PV容量÷PCS容量
    rekka = 0.007*0 #劣化率[1年あたり0.7%×経過年数](小数表記！！)
    effi_PCS = 1.0 #PCS効率(小数表記！！)
    effi_trans = 1.0   #トランス効率(小数表記！！)
    loss_haisen_teikaku=0.000    #定格出力時の配線ロス率(小数表記！！)
    ΔT = 18.4
    #裏面開放形(架台設置形):18.4,屋根置き形:21.5,屋根一体形:25.4,裏面密閉形(建材一体形):28.0
    #温度補正係数：1-(気温+ΔT-25)*0.31/100
    #例）屋根置き形だと、(21.5-18.4)*0.31/100=0.961(%)、出力を下げる
    #print('TaiyoKodo:',α,'TaiyoHoi:',A)
    kage_x=7 #ソーラーと影物体の直線距離(最短距離) 国富工場の環境：7m
    kage_y=100 #影物体の横の長さ 国富工場の環境：100m
    kage_h=0 #影物体の高さ。影物体が無い時は0にしておけばよい。国富工場の環境：12m(4階建て)
    kage_Φ=280 #ソーラーから見た時の影物体の方位。方位は北:0度、東:90度、南:180度、西:270度。国富工場の環境：280度
    #print(KageHandan(α,A,kage_x,kage_y,kage_h,kage_Φ))
